...
RESULTS_FOLDER = "results/linear_priority_buffer_results/"
MODEL_FOLDER = "models/"
...
class Buffer(object):
	...
	def priority_sample(self, state, next_state, reward, is_terminal):
		if len(self.buffer) >= self.min_buffer_size:
			if random.uniform(0, 1) > 0.5:
				return self.random_sample(state, next_state, reward, is_terminal)
			loss_list = [b[4] for b in self.buffer]
			max_loss_idx = np.argmax(loss_list)
			buffer_tuple = self.buffer[max_loss_idx]
			train_dict = {
					state: buffer_tuple[0], next_state: buffer_tuple[1],
					reward: buffer_tuple[2], is_terminal: buffer_tuple[3]
			}
			return True, max_loss_idx, train_dict
		return False, -1, None

##-----------------------------------------------------------
##-------- DQN Agent ----------------------------------------
class CrawlerAgent(object):
	def __init__(self, weights_shape, train_save_location, tf_model_folder, priority, gamma=0.99, learning_rate=0.01):
...
def main():
	...
	if args.run == "no-revisit":
		...
		model_save_file = MODEL_FOLDER + "linear_priority_buffer_model"
		results_save_file = RESULTS_FOLDER + "dqn_crawler_train_results.csv"
	else:
		revisit = True
		all_urls_file = RESULTS_FOLDER + "all_urls_revisit.csv"
		model_save_file = MODEL_FOLDER + "linear_priority_buffer_model_revisit"
	...
	agent = CrawlerAgent(weights_shape, results_save_file, model_save_file, priority, gamma=gamma, learning_rate=learning_rate)
